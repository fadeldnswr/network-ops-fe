...
export function CityCard({ city }: CityCardProps) {
  const offlinePercentage = ((city.ontOffline / city.totalOnt) * 100).toFixed(1)
  const onlinePercentage = ((city.ontOnline / city.totalOnt) * 100).toFixed(1)
  ...
  const getTotalTickets = () => city.ticketsInProgress.incident + city.ticketsInProgress.complain
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-3 gap-2">
          <div className="bg-secondary/40 rounded-lg p-2 border border-border/50">
            ...
            <p className="text-base font-bold text-foreground">{city.totalOnt}</p>
          </div>
          <div className="bg-green-500/10 rounded-lg p-2 border border-green-500/20">
            <p className="text-xs text-green-400 mb-0.5 font-medium">Online</p>
            <p className="text-base font-bold text-green-400">{city.ontOnline}</p>
          ...
          <div className="bg-red-500/10 rounded-lg p-2 border border-red-500/20">
            ...
            <p className="text-base font-bold text-red-400">{city.ontOffline}</p>
        ...
        <div className="flex items-center gap-3">
          <div className="flex-1">
            ...
            <div className="flex items-center gap-2">
              <div className="flex-1 bg-secondary rounded-full h-1.5">
                <div
                  className={`${getHealthColor(city.networkHealthScore)} bg-current h-1.5 rounded-full`}
                  style={{ width: `${city.networkHealthScore}%` }}
                />
              </div>
              <p className={`text-sm font-bold ${getHealthColor(city.networkHealthScore)}`}>
                {city.networkHealthScore}%
              </p>
            </div>
          </div>
          <div className="w-16 h-16 rounded-full flex items-center justify-center flex-shrink-0">
            <div
              ...
              style={{
                background: `conic-gradient(rgb(34, 197, 94) 0% ${city.networkHealthScore}%, rgb(107, 114, 128) ${city.networkHealthScore}% 100%)`,
                color: getHealthColor(city.networkHealthScore),
              }}
            >
              {city.networkHealthScore}
            </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="bg-orange-500/10 rounded-lg p-2 border border-orange-500/20">
              ...
              <p className="text-sm font-bold text-orange-600">{city.ticketsInProgress.incident}</p>
            </div>
            <div className="bg-blue-500/10 rounded-lg p-2 border border-blue-500/20">
              <p className="text-xs text-blue-600 mb-0.5">Complain</p>
              <p className="text-sm font-bold text-blue-600">{city.ticketsInProgress.complain}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-r from-primary/10 to-accent/10 rounded-lg p-3 border border-primary/20">
          ...
          <p className="text-xs text-foreground leading-relaxed">{city.aiPrescriptive}</p>
        </div>
      </div>
    </Card>
  )
